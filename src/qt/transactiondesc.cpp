...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    CRITICAL_BLOCK(wallet->cs_wallet)
    {
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(BitshekelUnits::formatWithUnit(BitshekelUnits::BTC, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,-nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += tr("<b>Debit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += tr("<b>Credit:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,wallet->GetCredit(txout)) + "<br>";
            }
        }

        strHTML += tr("<b>Net amount:</b> ") + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,nNet, true) + "<br>";
        ...
        if (fDebug)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if(wallet->IsMine(txin))
                    strHTML += "<b>Debit:</b> " + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if(wallet->IsMine(txout))
                    strHTML += "<b>Credit:</b> " + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,wallet->GetCredit(txout)) + "<br>";
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                {
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            strHTML = strHTML + " Amount=" + BitshekelUnits::formatWithUnit(BitshekelUnits::BTC,vout.nValue);
