...
{
    return QCoreApplication::translate("BitShekel-core", psz).toStdString();
}
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitshekelGUI::tr("A fatal error occured. Bitshekel can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}

#ifndef BITSHEKEL_QT_TEST
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if (strlen(argv[i]) > 8 && strncasecmp(argv[i], "BitShekel:", 9) == 0)
        {
            ...
            try {
                boost::interprocess::message_queue mq(boost::interprocess::open_only, "BitshekelURL");
    ...
    app.setOrganizationDomain("BitShekel.us");
    ...
    try
    {
        ...
        {
            {
                // Put this in a block, so that BitshekelGUI is cleaned up properly before
