...
// for both BitShekeld and BitShekel-qt, to make it harder for attackers to
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 8400000); // BitShekel: 84M blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // BitShekel: 3.5 days for diff retarget
static const int64 nTargetSpacing = 2.5 * 60; // BitShekel: 2.5 minutes between blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // BitShekel: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "BitShekel", wxOK | wxICON_EXCLAMATION);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BitShekel: increase each by adding 2 to BitShekel value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "addr")
    {
        ...
        // BitShekel: a temporary fix for the spam.
        if (vAddr.size() == 499)
        {
            pfrom->Misbehaving(100);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    CRITICAL_BLOCK(cs_mapTransactions)
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // BitShekeld: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    CRITICAL_BLOCK(cs_main)
    {
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitshekelMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BitshekelMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBitshekelMiner(void* parg);

static bool fGenerateBitshekelim = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static BitshekelMiner(CWallet *pwallet)
{
    printf("BitshekelMiner started\n");
    ...
    while (fGenerateBitshekelim)
    {
        if (AffinityBugWorkaround(ThreadBitshekelMiner))
            return;
        if (fShutdown)
            return;
        ...
        printf("Running BitshekelMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadBitshekelMiner(void* parg)
{
    ...
    try
    {
        ...
        BitshekelMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBitshekelMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBitshekelMiner()");
    }
    UIThreadCall(boost::bind(CalledSetStatusBar, "", 0));
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBitshekelMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBitshekelim(bool fGenerate, CWallet* pwallet)
{
    fGenerateBitshekelim = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBitshekelim = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BitshekelMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadBitshekelMiner, pwallet))
                printf("Error: CreateThread(ThreadBitshekelMiner) failed\n");
            Sleep(10);
        }
    }
}
