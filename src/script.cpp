...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // Bitshekel address tx, sender provides hash of pubkey, receiver provides signature and pubkey
...
bool Sign1(const CBitshekelAddress& address, const CKeyStore& keystore, uint256 hash, int nHashType, CScript& scriptSigRet)
{
    CKey key;
...
bool ExtractAddress(const CScript& scriptPubKey, CBitshekelAddress& addressRet)
{
    vector<valtype> vSolutions;
    txnouttype whichType;
    if (!Solver(scriptPubKey, whichType, vSolutions))
...
bool ExtractAddresses(const CScript& scriptPubKey, txnouttype& typeRet, vector<CBitshekelAddress>& addressRet, int& nRequiredRet)
{
    addressRet.clear();
    typeRet = TX_NONSTANDARD;
    vector<valtype> vSolutions;
...
void CScript::SetBitshekelAddress(const CBitshekelAddress& address)
{
    this->clear();
    if (address.IsScript())
