...
void dumpKeyInfo(uint256 privkey)
{
    ...
    for (int nCompressed=0; nCompressed<2; nCompressed++)
    {
        ...
        printf("    * address (base58): %s\n", CBitshekelAddress(vchPubKey).ToString().c_str());
    }
}
#endif
...
{
    CBitshekelSecret bsecret1, bsecret2, bsecret1C, bsecret2C;
    ...
    BOOST_CHECK(CBitshekelAddress(key1.GetPubKey ()).ToString() == "1QFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ");
    BOOST_CHECK(CBitshekelAddress(key2.GetPubKey ()).ToString() == "1F5y5E5FMc5YzdJtB9hLaUe43GDxEKXENJ");
    BOOST_CHECK(CBitshekelAddress(key1C.GetPubKey()).ToString() == "1NoJrossxPBKfCHuJXT4HadJrXRE9Fxiqs");
    BOOST_CHECK(CBitshekelAddress(key2C.GetPubKey()).ToString() == "1CRj2HyM1CXWzHAXLQtiGLyggNT9WQqsDs");
