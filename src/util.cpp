...
{
    // Windows: C:\Documents and Settings\username\Application Data\BitShekel
    // Mac: ~/Library/Application Support/BitShekel
    // Unix: ~/.BitShekel
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) + "\\BitShekel";
#else
...
string GetConfigFile()
{
    ...
    fs::path pathConfig(GetArg("-conf", "BitShekel.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return true; // No BitShekel.conf file is OK
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "BitShekel.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // BitShekel: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(ThreadSafeMessageBox, strMessage+" ", string("BitShekel"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }
        if (fDebug) {
